            ...
            {socialLinks.map(({ Icon, path }, index) => {
				return (
					<a href={path}
                        key={index}
                        target="_blank"
                        className="group relative w-12 h-12 p-3 rounded-full overflow-hidden hover:scale-110 transition-all duration-500"
                    >
                        ...
                        <FontAwesomeIcon icon={Icon} className="relative w-full h-full text-primary-300 z-10 group-hover:border-primary-100 group-hover:text-primary-900 transition-colors duration-500" />
                    </a>
				);
			})}
        </div>
